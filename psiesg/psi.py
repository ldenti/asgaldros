...
def psi(bam_path, ioe_path, out_path):
    ...
    for line in open(ioe_path):
        ...
        # sometimes, asgal may not align any read to a gene (hence not producing any alignment)
        ...
        bam_introns = bam.find_introns(
            (
                ...
                for read in bam.fetch(
                    chrom,
                    min(
                        intron1[0] if intron1[0] != 0 else float("inf"),
                        intron2[0] if intron2[0] != 0 else float("inf"),
                        intron3[0] if intron3[0] != 0 else float("inf"),
                        intron4[0] if intron4[0] != 0 else float("inf"),
                    )
                    - 50,
